...
function AuthModal({
  ...
  // حالات الـ OTP للنافذة
  const [isOtpStep, setIsOtpStep] = useState(false);
  const [otpValue, setOtpValue] = useState("");
  const [otpLoading, setOtpLoading] = useState(false);
  
  // حالات الدخول السريع (نسيت كلمة المرور)
  const [isLoginOtpMode, setIsLoginOtpMode] = useState(false);
  const [otpType, setOtpType] = useState<'signup' | 'email'>('signup');

  // تصفير الحقول عند إغلاق النافذة
  useEffect(() => {
    if (!isOpen) {
      setEmail("");
      setPassword("");
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl animate-fade-in">
        ...
        <div className="mt-2">
          <h2 className="mb-2 text-center text-xl font-bold tracking-tight text-foreground">
            {isOtpStep ? "Verify Your Email" : (isLoginOtpMode ? "Login Code" : "Sign In / Register")}
          </h2>
          ...
          {!isOtpStep ? (
            <>
              ...
              <form onSubmit={isLoginOtpMode ? handleSendLoginOtp : handleEmailAuth} className="flex flex-col gap-4">
                <div>
                  <input
                    type="email"
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground transition-all hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
                >
                  {isLoading 
                    ? "Processing..." 
                    : (isLoginOtpMode ? "Send Login Code" : (isSignUp ? "Sign Up" : "Sign In"))}
                </button>
